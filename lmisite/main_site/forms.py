...
class ContactForm(forms.Form):
    your_name = forms.CharField(
        label="Your name", widget=forms.TextInput(attrs={"placeholder": "Your name"})
    )
    your_email = forms.EmailField(
        label="Your email", widget=forms.EmailInput(attrs={"placeholder": "Your email"})
    )
    your_phone = PhoneNumberField(
    ...
    def render(self, name, value, attrs: dict = None, **kwargs):
        ...
        if is_signed_in:
            accounts = requests.get(
                "https://graph.facebook.com/v3.3/me/accounts",
...
class ConfigForm(forms.ModelForm):
    ...
    class Meta:
        ...
        widgets = {"facebook_page_id": PageWidget}
